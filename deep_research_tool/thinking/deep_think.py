...
抽出した事実:"""
    ...
    INFERENCE_PROMPT = """以下の前提（事実）に基づいて、Tree of Thought方式で多角的な推論を行ってください。
...
## 指示:
以下の4つのステップで推論を進めてください。

### Step 1: 思考の分岐（Branching）
前提から導き出せる可能性のある推論を、以下の3つの視点から生成してください：

**視点A - 直接的帰結**: 前提から論理的に直接導かれる結論は何か？
**視点B - 背景・文脈**: この事実が成り立つ背景や原因として考えられることは何か？
**視点C - 将来・影響**: この事実から予測される影響や展開は何か？

### Step 2: 各分岐の評価（Evaluation）
各視点の推論について、以下の基準で評価してください：
- 論理的妥当性（前提との整合性）: 高/中/低
- 根拠の強さ（飛躍の程度）: 高/中/低
- 有用性（洞察の価値）: 高/中/低

### Step 3: 最良経路の選択（Selection）
評価に基づき、最も信頼性が高く有用な推論を選択し、その理由を説明してください。
推論の深さレベルが高い({level} > 0.5)場合は、より探索的な推論も含めてください。
推論の深さレベルが低い({level} <= 0.5)場合は、保守的で確実な推論を優先してください。

### Step 4: 統合推論（Integration）
選択した推論を統合し、一貫した論理的結論を導出してください。
矛盾がある場合は、その矛盾を明示し、解決策を提案してください。
