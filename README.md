...
### LM Studioでタイムアウトする場合

Web UIの「設定 → 生成AI・プロキシ → ローカルLLM」で、バックエンドを **OpenAI互換**、サーバーURLを `http://127.0.0.1:1234/v1` に設定してください。モデル名はLM Studioに表示されている識別子を指定します。

- ローカルLLMでは生成途中の応答をストリーミングで受信します。生成全体に120秒以上かかっても、応答データが届いていれば受信を続けます。
- タイムアウトの既定値は **600秒**です。最初の応答待ちと受信途中の無通信時間に適用し、接続の確立は最大10秒で打ち切ります。Web UI、デスクトップ版のAPIタブ、CLIの `--local-timeout` で変更できます。
- 同時リクエストの既定値はローカルLLMクライアントごとに **1件**です。CPUやメモリに余裕がある場合は `--local-concurrency` などで調整できます。複数ジョブ・複数クライアント間の合計上限ではありません。
- 応答待ちのタイムアウトや受信途中の切断では、同じ生成を自動で再送しません。サーバー側で元の生成が残っている可能性があるため、LM Studio側の処理状況を確認してから再実行してください。
- JSONが途中で終わる場合は、タイムアウトとは別に、生成トークン上限と**モデル読み込み時のコンテキスト長**を確認してください。モデルが対応する最大長と、実際に読み込まれた長さは異なります。出力上限による打ち切りや不完全な調査計画は、正常な計画として採用しません。

参考：[LM StudioのChat Completions](https://lmstudio.ai/docs/developer/openai-compat/chat-completions)、[モデル情報・生成統計のAPI](https://lmstudio.ai/docs/developer/rest/endpoints)。
