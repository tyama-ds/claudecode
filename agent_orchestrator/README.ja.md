...
`write_file` / `run`（ワークスペースでシェル実行 — 信頼できる場合のみ）/ `http_get` /
`browser_get` を全エージェントに開放できます。Web系ツールは設定した **プロキシ** と
**User-Agent**（Settings で指定）を使います。`http_get` は標準ライブラリの高速取得
（JS非対応）、`browser_get` は**本物のヘッドレスブラウザ**でページを描画（Selenium →
Playwright → いずれも無ければ素のHTTPにフォールバック）するので、JSが重いサイトにも対応。
ブラウザの任意インストールは `requirements.txt` を参照。エージェントは返答中に `<TOOL name="run">pytest -q</TOOL>` と書くだけで、
...
ローカルLLMサーバは同時生成数に弱いことが多いため、*Local LLM* には**最大同時リクエスト数**
（`LOCAL_LLM_MAX_CONCURRENCY`、0＝無制限）の設定があります。ワーカー並列・レビュアー並列・
組織のレベル並列などの並列フェーズでも、ローカルLLMへの同時リクエストはこの数までに
絞られます（他のバックエンドはフル並列のまま）。
