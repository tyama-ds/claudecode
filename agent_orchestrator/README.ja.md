...
| **Org team（組織方式）** | 自由な指揮系統 | **マネージャー1人＋コンダクター2人＋ワーカー5人**のような階層を自分で設計。指示は階層を下へ（各レベルは並列実行）、報告は上へ流れ、トップの完了宣言で終了。指揮系統は**右パネルのグラフ上でインタラクティブに変更**（メンバー→新しい上司の順にクリック）、または各カードの「上司」プルダウンで設定。 |
