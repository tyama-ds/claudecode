...
| **Workspace build（実ファイル開発）** | implementer, reviewer | **実ディレクトリ上のペア開発**: まず2人で設計を相談・合意し、その後 implementer が実装、reviewer が差分を批評（軽微な修正は直接適用）。承認まで反復。 |
...
**Workspace build** はさらに一歩進み、2人のエージェントが**ディスク上の実ディレクトリで
ペア開発**します。最初に**設計相談（ラウンド0）**があり、実装役が方針を提案し、レビュー役が
リスクや代案を指摘して設計に合意してから、コードを書き始めます。その後は実装 ↔ レビューの
ループで、レビュー役も**軽微な修正なら直接ファイルに適用**できます。
